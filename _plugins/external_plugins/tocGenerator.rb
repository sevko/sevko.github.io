...
module Jekyll
	module TOCGenerator
		TOGGLE_HTML = '<div id="toctitle"><h2>%1</h2>%2</div>' unless const_defined?(:TOGGLE_HTML)
		TOC_CONTAINER_HTML = '<div id="toc-container"><table class="toc" id="toc"><tbody><tr><td>%1<ul>%2</ul></td></tr></tbody></table></div>' unless const_defined?(:TOC_CONTAINER_HTML)
		HIDE_HTML = '<span class="toctoggle">[<a id="toctogglelink" class="internal" href="#">%1</a>]</span>' unless const_defined?(:HIDE_HTML)
		...
		def toc_generate(html)
			...
			if toc_html.length > 0
				...
				doc.css('h1, h2').each do |header|
					toc_anchor = Nokogiri::XML::Node.new "a", doc
					toc_anchor['data-scroll'] = ''
					toc_anchor['href'] = '#toc-container'
					header.add_previous_sibling(toc_anchor)
					header.parent = toc_anchor
				end
				...
				doc.css('body').children.to_html(indent:3, indent_text:" ")
			else
				return html
			end
		end
		...
		def create_level_html(anchor_id, toc_level, toc_section, tocNumber, tocText, tocInner)
			link = '<a data-scroll href="#%1"><span class="tocnumber">%2</span> <span class="toctext">%3</span></a>%4'

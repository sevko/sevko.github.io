...
Note that the `parserState` argument passed to `nextChar()` is a `struct` containing the parser's state: a pointer to
the string being parsed, its length, the current index in that string, etc.
...
"nonlocal jumps to a saved stack context," or, in other words, allow you to perform jumps across functions . The gist
is that `setjmp()` is used to initialize a `jmp_buf`, storing critical information about the current calling
environment -- it's highly system-specific, but generally includes things like the stack pointer and current register
values -- and returns 0 (the **first** time it returns -- this will be explained shortly). You can then pass that `jmp_buf`
to `longjmp()` at any other point, and the program will rewind execution back to the `setjmp()` call. You'll also need
to pass a non-zero `int` to `longjmp()`, which will be the value that `setjmp()` returns this time around; this allows
us to discriminate between the times that `setjmp()` returns a.) initially and b.) after a jump was performed. An
example should set things straight:
...
to manually use pointer notation when necessary, but it is what it is.
...
Two of the solutions that came to mind were:
...
to all of the parse routines called by the current one. If the parse succeeds, just restore
...
perform the initial allocation of a stretchy array and then whenever a `realloc()` changes the location of the
allocated memory. When a long jump occurs, register values are restored from whatever they were at the time of the
`setjmp()` call, since those are what it copied into the target `jmp_buf`; `values` was `NULL` when `setjmp()` ran, so
that's the value that it would get assigned after a jump occurs.. **if** the compiler decided to put it into a
register. To prevent that from happening, we use the `volatile` specifier. See [this SO
...
If you're interested in more reading, I recommend [this fairly comprehensive
article](http://www.di.unipi.it/~nids/docs/longjump_try_trow_catch.html).

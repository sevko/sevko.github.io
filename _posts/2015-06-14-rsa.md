...
security [company](https://en.wikipedia.org/wiki/RSA_Security) of the same name -- confusing, but great PR!
...
introduces a number of interesting problems, like finding greatest common divisors, modular exponentiation, computing
modular inverses, and generating random prime numbers, each of which we'll thoroughly explore and derive solutions to
(many of these won't be immediately clear, so we'll formally prove them as we go). Note that we won't prove RSA itself
-- I might add that as an extension to the article at some point in the future.
...
Bingo. We've proven Case 2, and completed our proof of the Euclidean Algorithm.
...
We now prove assumption 2: if $$p$$ is prime and $$x ^ 2 \equiv 1 \pmod p$$, $$x$$ **must** equal $$\pm 1 \imod p$$.
First, note that if $$x \equiv \pm 1 \pmod p$$, then $$p \divs x \pm 1$$, so we can rewrite the assumption as: if $$p$$
is prime and $$x ^ 2 \equiv 1 \pmod p$$, then $$p \divs x \pm 1$$. We'll now prove it by proving its
[contrapositive](https://en.wikipedia.org/wiki/Contraposition), or: if $$p \nmid x + 1$$ and $$p \nmid x - 1$$, then
either $$p$$ is not a prime or $$x ^ 2 \not \equiv 1 \pmod p$$. If $$x \not \equiv \pm 1 \pmod p$$, then both of the
first two conditions are true, so **at least one** of the following must be true:

  1. $$p$$ is not a prime, or
  2. $x ^ 2 \not \equiv 1 \pmod p$

However, we're assuming that $$x ^ 2$$ *does* equal 1 $$\imod p$$, so 2.) is false and 1.) must be true as a
consequence. $$p$$ is not a prime. Having proven assumptions 1 and 2, we can move on.
...
since we usually want to create RSA keys of a specific bit-length.
...
primes $$p$$ and $$q$$ that we use to create it have a bit length of `bit-length / 2`, since multiplying two $$n$$-bit
...
b \imod c$$ by simply resolving $$a ^ b$$ and then simplifying that is a no-go. Fortunately, there are more efficient
ways of performing [modular exponentiation](https://en.wikipedia.org/wiki/Modular_exponentiation), like exponentiation
by squaring.
...
expression, we could maintain a local variable, say `accumulator`, that we'd initialize to $$a$$, and square from
...
one of the factors into the accumulator that'll become our ultimate result. Thus, `modular_power()` might look
something like this:

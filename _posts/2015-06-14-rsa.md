...
$$abx$$ is, by the lemma's requisite, divisible by $$p$$, and $$pby$$ is by definition divisible by $$p$$, so $$b$$
must be divisible by $$p$$ too.
...
We now prove assumption 2: if $$p$$ is prime and $$x ^ 2 \equiv 1 \pmod p$$, $$x$$ **must** equal ±1.  First, note that
if $$x \equiv \pm 1 \pmod p$$, then $$p \divs x \pm 1$$, so we can rewrite the assumption as: if $$p$$ is prime and $$x
^ 2 \equiv 1 \pmod p$$, then $$p \divs x \pm 1$$. We'll now prove it by proving its
[contrapositive](https://en.wikipedia.org/wiki/Contraposition), or: if $$p \nmid x + 1$$ and $$p \nmid x - 1$$, then
either $$p$$ is not a prime or $$x ^ 2 \not \equiv 1 \pmod p$$. If $$x \not \equiv \pm 1 \pmod p$$, then both of
the first two conditions are true, so **at least one** of the following must be true:
